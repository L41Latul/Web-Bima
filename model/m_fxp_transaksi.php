...
$select = "SELECT id_transaksi, DATE_FORMAT(tgl_transaksi, '%d-%m-%Y') AS PerDate, jt.jenis_transaksi, c.nama, b.bank, i.nama_ib, CONCAT('Rp. ', FORMAT(jumlah_transaksi, 0)) as jml_in, CONCAT('Rp. ', FORMAT(transfer_ke, 0)) as jml_out, CONCAT('Rp. ', FORMAT(laba_rugi, 0)) as labarugi FROM fxp_transaksi m LEFT JOIN jenis_transaksi jt ON m.id_jenis_transaksi = jt.id_jenis_transaksi LEFT JOIN bank b ON m.id_bank = b.id_bank LEFT JOIN fxp_client c ON m.id_client = c.id_client LEFT JOIN fxp_ib i ON m.id_ib = i.id_ib";
$where = ' WHERE TRUE';
...
$select2 = "select jenis_transaksi, CONCAT('Rp. ', FORMAT(sum(jumlah_transaksi), 0)) as jml_in, CONCAT('Rp. ', FORMAT(sum(transfer_ke), 0)) as jml_out, CONCAT('Rp. ', FORMAT(sum(laba_rugi), 0)) as jml_laba FROM fxp_transaksi m LEFT JOIN jenis_transaksi jt ON m.id_jenis_transaksi = jt.id_jenis_transaksi LEFT JOIN bank b ON m.id_bank = b.id_bank LEFT JOIN fxp_client c ON m.id_client = c.id_client LEFT JOIN fxp_ib i ON m.id_ib = i.id_ib";
...
$select3 = "select CONCAT('Rp. ', FORMAT(sum(jumlah_transaksi), 0)) as jml_in, CONCAT('Rp. ', FORMAT(sum(transfer_ke), 0)) as jml_out, CONCAT('Rp. ', FORMAT(sum(laba_rugi), 0)) as jml_laba FROM fxp_transaksi m LEFT JOIN jenis_transaksi jt ON m.id_jenis_transaksi = jt.id_jenis_transaksi LEFT JOIN bank b ON m.id_bank = b.id_bank LEFT JOIN fxp_client c ON m.id_client = c.id_client LEFT JOIN fxp_ib i ON m.id_ib = i.id_ib";
$sql3 = $select3 . $where;
...
echo '{"users":'.json_encode($results).',  "jumlah" : '.$counts.',  "sums":'.json_encode($results2).',  "sums2":'.json_encode($results3).',  "query" : " '.$sql.'"}';
?>