...
$select = "SELECT m.id_ib, m.nama_ib, m.alamat, m.no_telp, m.email, m.pin_bb, bank, count(c.id_client) as jumlah_client FROM fxp_ib m LEFT JOIN bank b on m.id_bank = b.id_bank LEFT JOIN fxp_client c ON m.id_ib = c.id_ib";
$where = ' WHERE TRUE';
...
echo '{"fxp_ib":'.json_encode($results).',  "jumlah" : '.$counts.',  "query" : " '.$sql.'"}';
?>